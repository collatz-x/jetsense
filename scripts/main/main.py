...
def process_silver_layer(spark, train_paths=None):
    ...
    """
    if train_paths is None:
        train_paths = ['bronze_new_input']
    
    print("\n⚪ Processing Silver Layer...")
    # Combine all Bronze datasets into unified Silver table
    df = sp.process_silver_table(train_paths, "datamart/bronze_inference/", "datamart/silver_inference", spark)
    return df
...
def clean_silver_features(spark, silver_path="datamart/silver_inference/silver_feature.parquet"):
    """
    Silver Layer: Feature Selection and Cleanup
    ...
    cleaned_path = "datamart/silver_inference/silver_feature_cleaned.parquet"
...
def process_gold_layer(spark, silver_filepath, gold_directory="datamart/gold_inference"):
    """
    Gold Layer: Feature Engineering and ML Preparation
...
def print_pipeline_summary(gold_directory="datamart/gold_inference"):
    """
